...
export default function MissionModal({ mission, onClose, onComplete }: MissionModalProps) {
  ...
  const handleDragStart = (event: DragStartEvent) => {
    ...
    if (data?.block) setDraggedItem({ block: data.block, type: data.type });
  };
  ...
  const handleDragEnd = (event: DragEndEvent) => {
    ...
    if (type === "sensor" && !isAnalog && !["D2", "D3"].includes(pinId)) {
      setGuideMsg(`💡 Sensors work best on analog pins (A0-A5). Try the bottom pins!`);
      return;
    }
    if (type === "actuator" && isAnalog) {
      setGuideMsg(`💡 Actuators need digital pins (D4-D13). Try the side pins!`);
      return;
    }

    setConnections((prev) => [...prev, { pinId, componentId: block.id, componentName: block.name, componentIcon: block.icon, type }]);
    setGuideMsg(`✅ ${block.icon} ${block.name} → pin ${pinId}! ${type === "sensor" ? "Input ready!" : "Output ready!"} 🔌`);
  };
  ...
  // Validate that the right sensors and actuators are connected for this mission
  const validateCircuit = useCallback((): string[] => {
    const errors: string[] = [];
    const connectedSensorNames = connections.filter((c) => c.type === "sensor").map((c) => c.componentName);
    const connectedActuatorNames = connections.filter((c) => c.type === "actuator").map((c) => c.componentName);

    mission.requiredSensors.forEach((req) => {
      if (!connectedSensorNames.includes(req)) {
        errors.push(`🔴 Missing sensor: ${req}`);
      }
    });
    mission.requiredActuators.forEach((req) => {
      if (!connectedActuatorNames.includes(req)) {
        errors.push(`🔴 Missing actuator: ${req}`);
      }
    });

    // Check sensors are on analog pins
    connections.filter((c) => c.type === "sensor").forEach((c) => {
      if (!c.pinId.startsWith("A") && !["D2", "D3"].includes(c.pinId)) {
        errors.push(`⚠️ ${c.componentName} should be on an analog pin (A0-A5)`);
      }
    });

    // Check actuators are on digital pins
    connections.filter((c) => c.type === "actuator").forEach((c) => {
      if (c.pinId.startsWith("A")) {
        errors.push(`⚠️ ${c.componentName} should be on a digital pin (D4-D13)`);
      }
    });
  ...
  const handleSubmit = () => {
    const circuitErrors = validateCircuit();

    if (circuitErrors.length > 0) {
      setGuideMsg(`🔧 Fix your circuit first:\n${circuitErrors.join("\n")}`);
      return;
    }

    if (!codeValid) {
      setGuideMsg("🧩 Switch to the Program tab and build the correct code logic! You need: Read sensor → Check condition → Set output.");
      setLabTab("code");
      return;
    }

    setStep("success");
    setGuideMsg("🎉 AMAZING! Your circuit and code are correct! The city is safer thanks to you!");
  };
  ...
  return (
    <AnimatePresence>
      <motion.div className="fixed inset-0 z-50 flex items-center justify-center p-4" initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }}>
        ...
        >
          <button onClick={onClose} className="absolute top-4 right-4 w-10 h-10 rounded-full bg-muted flex items-center justify-center text-lg hover:bg-destructive hover:text-destructive-foreground transition-colors z-10">✕</button>

          {/* INTRO */}
          {step === "intro" && (
            <div className="space-y-6">
              <div className="flex items-start gap-4">
                <div className="w-16 h-16 rounded-2xl bg-primary flex items-center justify-center text-3xl">{mission.icon}</div>
                <div>
                  <h2 className="font-display text-2xl font-extrabold text-foreground">{mission.title}</h2>
                  <div className="flex items-center gap-3 mt-1">
                    <span className={`${difficultyStyles.bg} px-3 py-1 rounded-full text-xs font-bold text-primary-foreground`}>{difficultyStyles.label}</span>
              ...
              <div className="bg-primary/5 rounded-2xl p-4">
                <h4 className="font-display text-sm font-bold text-foreground mb-2">🔧 Required Components</h4>
                <div className="flex flex-wrap gap-2">
                  {mission.requiredSensors.map((s) => (
                    <span key={s} className="bg-secondary/20 text-secondary px-2 py-1 rounded-lg text-xs font-bold">📡 {s}</span>
                  ))}
                  {mission.requiredActuators.map((a) => (
                    <span key={a} className="bg-accent/20 text-accent px-2 py-1 rounded-lg text-xs font-bold">⚡ {a}</span>
                  ))}
                </div>
              </div>
              ...
              <button
                ...
                onClick={() => {
                  setStep("lab");
                  setGuideMsg("Drag the required sensors & actuators onto the Arduino pins, then program the logic! 🛠️");
                }}
              >
                🚀 Start Lab
              </button>
          ...
          {step === "lab" && (
            <div className="space-y-4">
              ...
              <div className="bg-muted rounded-xl p-3">
                <h4 className="text-xs font-display font-bold text-foreground mb-2">✅ Checklist</h4>
                <div className="grid grid-cols-2 gap-1">
                  {mission.requiredSensors.map((s) => {
                    const connected = connections.some((c) => c.componentName === s && c.type === "sensor");
                    return (
                      <div key={s} className={`flex items-center gap-1.5 text-xs font-body font-semibold ${connected ? "text-secondary" : "text-muted-foreground"}`}>
                        <span>{connected ? "✅" : "⬜"}</span> {s}
                      </div>
                    );
                  })}
                  {mission.requiredActuators.map((a) => {
                    const connected = connections.some((c) => c.componentName === a && c.type === "actuator");
                    return (
                      <div key={a} className={`flex items-center gap-1.5 text-xs font-body font-semibold ${connected ? "text-accent" : "text-muted-foreground"}`}>
                        <span>{connected ? "✅" : "⬜"}</span> {a}
                      </div>
                    );
                  })}
                  <div className={`flex items-center gap-1.5 text-xs font-body font-semibold ${codeValid ? "text-secondary" : "text-muted-foreground"}`}>
                    <span>{codeValid ? "✅" : "⬜"}</span> Code logic
                  </div>
                </div>
              </div>
              ...
              <div className="flex gap-2">
                <button
                  ...
                  className={`flex-1 py-2 rounded-xl font-display font-bold text-sm transition-all ${labTab === "build" ? "bg-primary text-primary-foreground" : "bg-muted text-muted-foreground hover:bg-muted/80"}`}
                >
                  🔧 Build Circuit
                </button>
                <button
                  onClick={() => setLabTab("code")}
                  className={`flex-1 py-2 rounded-xl font-display font-bold text-sm transition-all ${labTab === "code" ? "bg-primary text-primary-foreground" : "bg-muted text-muted-foreground hover:bg-muted/80"}`}
                >
                  🧩 Program
                </button>
              </div>
                      ...
                      {connections.length > 0 && (
                        <div className="bg-muted rounded-xl p-3">
                          ...
                          <div className="space-y-1">
                            {connections.map((c) => (
                              <div key={c.pinId} className="flex items-center justify-between bg-card rounded-lg px-3 py-2 border border-border group cursor-pointer hover:border-destructive transition-colors" onClick={() => removeConnection(c.pinId)}>
                                <div className="flex items-center gap-2">
                                  <span className="font-mono text-[10px] bg-muted px-1.5 py-0.5 rounded font-bold text-muted-foreground">{c.pinId}</span>
              ...
              <div className="flex items-center gap-3">
                ...
                <button className="game-btn-primary flex-1 text-lg" onClick={handleSubmit}>✅ Submit Solution</button>
              </div>
            </div>
          )}
          ...
          {step === "success" && (
            <motion.div className="text-center space-y-6 py-6" initial={{ scale: 0.8 }} animate={{ scale: 1 }}>
              <motion.div className="text-7xl" animate={{ rotate: [0, -10, 10, -10, 0], scale: [1, 1.2, 1] }} transition={{ duration: 0.5 }}>🎉</motion.div>
              ...
              <button className="game-btn-secondary text-xl" onClick={handleCelebrate}>🏆 Collect Reward</button>
            </motion.div>
          )}
        </motion.div>
      </motion.div>
